...
const generateRandomPrices = (count, minDifference, maxDifference, originalPrice) => {
    const randomPrices = [];
  
    let currentPrice = originalPrice;
  
    for (let i = 0; i < count - 1; i++) {
      // Ensure the random difference is within the specified range and does not exceed half of the original price
      const maxIncrease = Math.min(maxDifference, originalPrice / 2 - currentPrice);
      const maxDecrease = Math.min(maxDifference, currentPrice - originalPrice / 2);
  
      const randomDifference = Math.floor(Math.random() * (maxIncrease - minDifference + 1)) + minDifference;
      currentPrice = Math.random() > 0.5 ? currentPrice + randomDifference : currentPrice - randomDifference;
  
      // Ensure the price stays within the specified range around the original price
      currentPrice = Math.max(originalPrice / 2, Math.min(originalPrice * 1.5, currentPrice));
  
      randomPrices.push(currentPrice);
    }
  
    // Set the last value as the original price
    randomPrices.push(originalPrice);
...
const Details = () => {
   ...
   const toggleChart = () => {
    ...
    useEffect(() => {
      // Assuming product.price is the current product's price
      if (price) {
        setprice(price);
      }
    }, [product]);
    const discountedPrice = product.price - Math.floor((product.price * product.discount) / 100);
const originalPrice = product.discount !== 0 ? discountedPrice : product.price;
    
    const chartState = {
        series: [
          {
            name: "Discount in '%'",
            data: [10, 30, 34, 20, 25, 40, 10, 15, 30, 5,18 , 40]
          },
          
          {
            name: "Price in '₹'",
            data: generateRandomPrices(12, 50, 100, originalPrice)
          }
        ],
        options: {
          color: ['#181ee8', '#181ee8'],
                ...
                <div className='absolute left-0 top-0 w-full h-full bg-[#2422228a]'>
                    <div className='w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto'>
                        <div className='flex flex-col justify-center gap-1 items-center h-full w-full text-white'>
                            <h2 className='text-3xl font-bold'>Shop Stop</h2>
                        </div>
                    </div>
                </div>
            </div>
